<?php
/**
 * File containing the NewsletterEditionSendItem class
 *
...
class NewsletterEditionSendItem extends eZPersistentObject
{
    const STATUS_NEW = 0;
    const STATUS_SEND = 1;
     ...
     * @return void
     */
    function NewsletterEditionSendItem( $row )
    {
    ...
    static function definition()
    {
        return array( 'fields' => array( 'id' => array( 'name' => 'Id',
                      ...
                      'class_name' => 'NewsletterEditionSendItem',
                      'name' => 'nl_edition_send_item' );
    }
    ...
    {
        $existingObject = NewsletterEditionSendItem::fetchBySendIdAndNewsletterUserIdAndOutputFormatId( $editionSendId, $newsletterUserId, $outputFormatId );
        ...
        else
        {
            $rows = array(      'edition_send_id' => (int) $editionSendId,
                                ...
                                'hash' => NewsletterUtils::generateUniqueMd5Hash( $editionSendId. '-'. $newsletterUserId ),

                                );

            $object = new NewsletterEditionSendItem( $rows );

            $object->setAttribute( 'status', NewsletterEditionSendItem::STATUS_NEW );
            $object->store();
    ...
    function setAttribute( $attr, $value )
    {
        switch ( $attr )
        {
            case 'status':
            {
                switch ( $value )
                {
                    case NewsletterEditionSendItem::STATUS_NEW:
                        $this->setAttribute('created', time() );
                    break;

                    case NewsletterEditionSendItem::STATUS_SEND:
                        $this->setAttribute('processed', time() );
                    break;

                    case NewsletterEditionSendItem::STATUS_ABORT:
                        $this->setAttribute('processed', time() );
                    break;
                ...
                NewsletterLog::writeDebug(
                                            'set status - NewsletterEditionSendItem::setAttribute',
                                            'send_item',
                                             $this->attribute('id'),
    ...
    function getStatusString()
    {
        ...
        switch( $this->attribute('status') )
        {
            case self::STATUS_NEW:
                $statusString = ezpI18n::tr( 'newsletter/editionsenditem/status', 'New' );
                break;
            case self::STATUS_SEND:
                $statusString = ezpI18n::tr( 'newsletter/editionsenditem/status', 'Send' );
                break;
            case self::STATUS_ABORT:
                $statusString = ezpI18n::tr( 'newsletter/editionsenditem/status', 'Abort' );
                break;
        }
    ...
    static function fetchListByNewsletterIdCount( $newsletterUserId )
    {
        ...
        if ( is_numeric( $newsletterUserId ) )
        {
            ...
            $sql = "SELECT count(*) AS count FROM nl_edition_send_item WHERE newsletter_user_id=$newsletterUserId";
    ...
    {
        $objectList = eZPersistentObject::fetchObjectList(
                                                    NewsletterEditionSendItem::definition(),
                                                    null,
    ...
    static function fetchListBySendIdAndStatusCount( $editionSendId, $editionSendItemStatus, $asObject = true )
    {
        ...
        if ( is_numeric( $editionSendId ) )
        {
            ...
            $sql = "SELECT count(*) AS count FROM nl_edition_send_item WHERE edition_send_id=$editionSendId";
    ...
    static function fetchListByStatusCount( $editionSendItemStatus, $asObject = true )
    {
        ...
        if ( is_numeric( $editionSendItemStatus ) )
        {
            ...
            $sql = "SELECT count(*) AS count FROM nl_edition_send_item WHERE status=$editionSendItemStatus";
            $countArray = $db->arrayQuery( $sql );
            $count = (int) $countArray[0]['count'];
        }
        return $count;
    ...
    {
        return NewsletterUser::fetch( $this->attribute('newsletter_user_id') );
    }
    ...
    {
        return NewsletterSubscription::fetch( $this->attribute('subscription_id') );
    }
