<?php
/**
 * File containing the NewsletterImport class
 *
 ...
 * @package newsletter
 */
class NewsletterImport extends eZPersistentObject
{
    ...
    public static function create( $listContentObjectId, $type = 'default', $note = '', $dataText = '', $remoteId = false )
    {
        if( $remoteId === false )
        {
            $remoteId = $type . ':'. NewsletterUtils::generateUniqueMd5Hash( $listContentObjectId );
        }
        ...
        $newObject = new NewsletterImport( $row );

        return $newObject;
    }

    /**
     * fetch NewsletterImport object by id
     * return false if not found
     *
     * @param integer $id
     * @param boolean $asObject
     * @return NewsletterMailboxItem or false
     */
    public static function fetch( $id, $asObject = true )
    {
         return eZPersistentObject::fetchObject(
                                                    NewsletterImport::definition(),
                                                    null,
                                                    array( 'id' => (int) $id ),
                                                    $asObject
                                                );
    ...
    /**
     * fetch NewsletterImport object by remoteId
     * return false if not found
     *
    ...
    {
        return NewsletterSubscription::fetchSubscriptionListByImportIdCount( $this->attribute('id') );
    }
    ...
    {
        return NewsletterSubscription::fetchSubscriptionListByImportIdAndStatusCount( $this->attribute('id'), NewsletterSubscription::STATUS_APPROVED );
    }
    ...
    {
        return NewsletterUser::fetchUserListByImportIdCount( $this->attribute('id') );
    }
    ...
    {
        return NewsletterUser::fetchUserListByImportIdAndStatusCount( $this->attribute('id'), NewsletterUser::STATUS_CONFIRMED );
    }
    ...
    public function removeActiveSubscriptionsByAdmin()
    {
        ...
        $count = NewsletterSubscription::fetchSubscriptionListByImportIdAndStatusCount( $this->attribute('id'), NewsletterSubscription::STATUS_APPROVED );

        NewsletterLog::writeNotice(
                                            "NewsletterImport::removeActiveSubscriptionsByAdmin",
                                            'import',
                                            'start',
        ...
        for ( $i = 0; $i < $loops; $i++ )
        {
            ...
            $subscriptionObjectList = NewsletterSubscription::fetchSubscriptionListByImportIdAndStatus( $this->attribute('id'), NewsletterSubscription::STATUS_APPROVED, $limit );
