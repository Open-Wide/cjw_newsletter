<?php
/**
 * File containing the NewsletterSubscription class
 *
...
class NewsletterSubscription extends eZPersistentObject
{
     ...
     * @return void
     */
    function NewsletterSubscription( $row )
    {
    ...
    static function definition()
    {
        return array( 'fields' => array(
                      ...
                      'increment_key' => 'id',
                      'sort' => array( 'id' => 'asc' ),
                      'class_name' => 'NewsletterSubscription',
                      'name' => 'nl_subscription' );
    }

    /**
     * Create new NewsletterSubscription object
     *
                            ...
                            $context = 'default' )
    {
        $rows = array( 'created' => time(),
                       ...
                       'output_format_array_string' => NewsletterSubscription::arrayToString( $outputFormatArray ),
                       'creator_contentobject_id' => eZUser::currentUserID(),
                       'hash' => NewsletterUtils::generateUniqueMd5Hash( $newsletterUserId ),
                       'remote_id' => 'nl:'. $context .':' .NewsletterUtils::generateUniqueMd5Hash( $newsletterUserId ),
                       'status' => 0 );

        $object = new NewsletterSubscription( $rows );
    ...
    function setAttribute( $attr, $value )
    {
        switch ( $attr )
        {
            case 'status':
            {
                ...
                switch ( $value )
                {
                    case NewsletterSubscription::STATUS_CONFIRMED :
                    {
                        ...
                        if ( is_object( $newsletterListAttributeContent ) and (int) $newsletterListAttributeContent->attribute('auto_approve_registered_user') == 1 )
                        {
                            ...
                            $value = NewsletterSubscription::STATUS_APPROVED;
                        }
                        else
                        {
                    ...
                    case NewsletterSubscription::STATUS_APPROVED:
                    {
                        $this->setAttribute( 'approved', $currentTimeStamp );
                        $this->setAttribute( 'removed', 0 );
                    } break;

                    case NewsletterSubscription::STATUS_REMOVED_ADMIN:
                    case NewsletterSubscription::STATUS_REMOVED_SELF:
                    {
                ...
                if( $statusOld != $statusNew )
                {
                    ...
                    NewsletterLog::writeNotice(
                                                'NewsletterSubscription::setAttribute',
                                                'subscription',
                                                'status',
    ...
    function isRemoved()
    {
        ...
        if ( $subscriptionStatus == NewsletterSubscription::STATUS_REMOVED_ADMIN
            || $subscriptionStatus == NewsletterSubscription::STATUS_REMOVED_SELF )
            return true;
        else
            return false;
    }
    ...
    {
        $subscriptionStatus = $this->attribute('status');
        if ( $subscriptionStatus == NewsletterSubscription::STATUS_REMOVED_SELF )
            return true;
        else
            return false;
    }
    ...
    {
        $subscriptionStatus = $this->attribute('status');
        if ( $subscriptionStatus == NewsletterSubscription::STATUS_BLACKLISTED )
            return true;
        else
            return false;
    }
    ...
    {
        $userObject = NewsletterUser::fetch( $this->attribute('newsletter_user_id'));
        return $userObject;
    }
     ...
     * used by confirmAll in NewsletterUser->confirmAll
     *
     * @return boolean
     */
    function confirm()
    {
        if ( $this->attribute('status') == NewsletterSubscription::STATUS_PENDING )
        {
            // timestamp + status setzen
            $this->setAttribute('status', NewsletterSubscription::STATUS_CONFIRMED );
            $this->store();
            return true;
        }
        if ( $this->attribute('status') == NewsletterSubscription::STATUS_APPROVED )
        {
            // timestamp + status setzen
    ...
    {
        //$this->setAttribute( 'output_format_array_string', NewsletterSubscription::arrayToString(  array() ) );
        if ( $this->attribute('status') == NewsletterSubscription::STATUS_BLACKLISTED
          || $this->attribute('status') == NewsletterSubscription::STATUS_REMOVED_SELF )
        {
            return false;
        }
        else
        {
            $this->setAttribute( 'status', NewsletterSubscription::STATUS_REMOVED_SELF );
            $this->sync();
            $this->store();
            return true;
        }
    ...
    {
        $this->setAttribute( 'status', NewsletterSubscription::STATUS_REMOVED_ADMIN );
        $this->sync();
        $this->store();
    }
    ...
    {
        $this->setAttribute( 'status', NewsletterSubscription::STATUS_APPROVED );
        $this->sync();
        $this->store();
    }
     ...
     * @param $newNewsletterUserStatus status for new created nl users e.g. NewsletterUser::STATUS_PENDING
                                               ...
                                               $context = 'default' )
    {
        ...
        $checkResult = NewsletterUser::checkIfUserCanBeUpdated( $email, $eZUserId, $updateNewEmail = true );
        switch ( $checkResult )
        {
            // create new user
            case 40:
            ...
            case -1:
                ...
                {
                    eZDebug::writeDebug( "checkResult[$checkResult] - NewsletterSubscription::createSubscriptionByArray return false because email already exists" );
        ...
        $newsletterUserObject = NewsletterUser::createUpdateNewsletterUser( $email,
                                                                               $salutation,
                                                                               $firstName,
                                                                               $lastName,
                                                                               $organisation,
        ...
        foreach ( $listArray as $listId )
        {
            ...
            $status = NewsletterSubscription::STATUS_PENDING;
            $dryRun = false;
            $resultArray['list_subscribe'][ $listId ] = NewsletterSubscription::createUpdateNewsletterSubscription(
                                                                                                                    $listId,
                                                                                                                    $newsletterUserId,
                                                                                                                    $outputFormatArray,
                                                                                                                    $status,
        ...
        if ( $subscribeOnlyMode === false )
        {
            ...
            {
                $resultArray['list_remove'][ $listId ] = NewsletterSubscription::removeSubscriptionByNewsletterUserSelf( $listId, $newsletterUserId );
            }
        }
        return $resultArray;
    }
    ...
    {
        $existingSubscriptionObject = NewsletterSubscription::fetchByListIdAndNewsletterUserId( $listContentObjectId, $newsletterUserId );
    ...
    */
    ...
    {
        $existingSubscriptionObject = NewsletterSubscription::fetchByListIdAndNewsletterUserId( $listContentObjectId, $newsletterUserId );
        $newsletterUser = NewsletterUser::fetch( $newsletterUserId );

        // if nl user status is confirmed set all nl subscription with status pending to confirmed
        if ( is_object( $newsletterUser )
            && (int) $newsletterUser->attribute('status') == NewsletterUser::STATUS_CONFIRMED
            && $status == NewsletterSubscription::STATUS_PENDING )
        {
            $status = NewsletterSubscription::STATUS_CONFIRMED;
        }
        ...
        if ( is_object( $existingSubscriptionObject ) )
        {
            ...
            $existingSubscriptionObject->setAttribute('output_format_array_string', NewsletterSubscription::arrayToString( $outputFormatArray ) );
            if( $context == 'configure' )
            {
                ...
                if( $existingSubscriptionObject->attribute( 'status' ) == NewsletterSubscription::STATUS_APPROVED )
                {
        ...
        {
            $object = NewsletterSubscription::create( $listContentObjectId,
                                                         $newsletterUserId,
                                                         $outputFormatArray,
                                                         $status,
                                                         $context );
    ...
    {
        $objectList = eZPersistentObject::fetchObjectList( NewsletterSubscription::definition(),
                                                    null,
    ...
    static function fetchSubscriptionListByListId( $listContentObjectId, $statusId = false, $limit = 50, $offset = 0, $asObject = true )
    {
        ...
        $objectList = eZPersistentObject::fetchObjectList(
                                                    NewsletterSubscription::definition(),
                                                    null,
                                                    $condArr,
                                                    $sortArr,
                                                    $limitArr,
    ...
    static function fetchListNotRemovedOrBlacklistedByNewsletterUserId( $newsletterUserId, $asObject = true )
    {
        $objectList = eZPersistentObject::fetchObjectList(
                        ...
                        null,
                        array( 'newsletter_user_id' => (int) $newsletterUserId,
                               'status' => array( array( NewsletterSubscription::STATUS_PENDING,
                                                         NewsletterSubscription::STATUS_CONFIRMED,
                                                         NewsletterSubscription::STATUS_APPROVED,
                                                         NewsletterSubscription::STATUS_BOUNCED_SOFT,
                                                         NewsletterSubscription::STATUS_BOUNCED_HARD ) ) ),
                        null,
                        null,
                        true
                        );
    ...
    {
        return eZPersistentObject::fetchObject( NewsletterSubscription::definition(),
                                                null,
                                                array( 'hash' => $hash ),
                                                $asObject );
    }
