<?php
/**
 * File containing the NewsletterEdition class
 *
 ...
 */
/**
 * Data management datatyp newsletteredition
 *
 * @version //autogentag//
 * @package newsletter
 */
class NewsletterEdition extends eZPersistentObject
{
     ...
     * @return void
     */
    function NewsletterEdition( $row )
    {
    ...
    {
        if ( $this->isProcess() )
            return NewsletterEdition::STATUS_PROCESS;

        if ( $this->isArchive() )
            return NewsletterEdition::STATUS_ARCHIVE;

        if ( $this->isAbort() )
            return NewsletterEdition::STATUS_ABORT;

        if ( $this->isDraft() )
            return NewsletterEdition::STATUS_DRAFT;
    ...
    function isDraft()
    {
        return $this->getEditionStatus( NewsletterEdition::STATUS_DRAFT );
    }
    ...
    function isProcess()
    {
        return $this->getEditionStatus( NewsletterEdition::STATUS_PROCESS );
    }
    ...
    function isArchive()
    {
        return $this->getEditionStatus( NewsletterEdition::STATUS_ARCHIVE );
    }
    ...
    function isAbort()
    {
         return $this->getEditionStatus( NewsletterEdition::STATUS_ABORT );
    }
    ...
    */
    ...
    function getEditionStatus( $virtualStatus )
    {
        ...
        switch ( $virtualStatus )
        {
             ...
             */
            case NewsletterEdition::STATUS_PROCESS :
            {
                 // if is a object version in sendprocess, it's can be WAIT_FOR_PROCESS or PROCESS
                $processResult = NewsletterEditionSend::fetchByEditionContentObjectIdAndStatus(
                                            $editionContentObjectId,
                                            array( NewsletterEditionSend::STATUS_WAIT_FOR_PROCESS,
                                                   NewsletterEditionSend::STATUS_MAILQUEUE_CREATED,
                                                   NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_STARTED  ),
                                            false
                                                   );
            ...
            */
            case NewsletterEdition::STATUS_DRAFT :
            {
                // if hasn't the current version a entry in edition_send => DRAFT
                $processResult = NewsletterEditionSend::fetchByEditionContentObjectIdVersion(
                                            $editionContentObjectId,
            ...
            case NewsletterEdition::STATUS_ARCHIVE :
            {
                // if sending of current version successful => ARCHIVE
                /*
                $processResult = NewsletterEditionSend::fetchByEditionContentObjectIdVersionAndStatus(
                                            $editionContentObjectId,
                                            $editionContentObjectIdVersion,
                                            array( NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_FINISHED ),
                                            false
                                            );
                */
            ...
            case NewsletterEdition::STATUS_ABORT :
            {
                ...
                $processResult = NewsletterEditionSend::fetchByEditionContentObjectIdVersionAndStatus(
                                            $editionContentObjectId,
                                            $editionContentObjectIdVersion,
                                            array( NewsletterEditionSend::STATUS_ABORT ),
                                            false
                                            );
                 */
    ...
    function getListAttributeContent()
    {
        ...
        {
            if ( $attribute->attribute('data_type_string') == 'newsletterlist' )
            {
    ...
    {
        $objectArray = array( 'all' => NewsletterEditionSend::fetchByEditionContentObjectId(
                                                    $this->attribute('contentobject_id')
                                                    ),
                              'current' => NewsletterEditionSend::fetchByEditionContentObjectIdVersion(
                                                    $this->attribute('contentobject_id'),
    ...
    {
        $objectArray = NewsletterEditionSend::fetchByEditionContentObjectIdVersion(
                                                    $this->attribute('contentobject_id'),
                                                    $this->attribute('contentobject_attribute_version')
                                                    );
    ...
    function createNewsletterSendObject()
    {
        ...
        $sendObject = NewsletterEditionSend::create( $this );
        $sendObject->store();
    ...
    function createOutputXml()
    {
        ...
        foreach ( $outputFormatArray  as $id => $name )
        {
            ...
            $textArray = NewsletterEdition::getOutput( $editionContentObjectId, $editionContentObjectVersion, $id, $mainSiteAccess, $skinName, $forceNotIncludingImages );
     ...
     *
     * @see classes/newslettermail.php getAllOutputFormatTextByContentObjectVersion()
    ...
    static function getOutput( $editionContentObjectId, $versionId, $outputFormat, $siteAccess, $skinName = 'default', $forceSettingImageIncludeTo = -1 )
    {
        ...
        $cmd = "\"$phpCli\" extension/newsletter/bin/php/createoutput.php --object_id=$editionContentObjectId --object_version=$versionId --output_format_id=$outputFormat $wwwDirString--current_hostname=$currentHostName --skin_name=$skinName -s $siteAccess";
        ...
        {
            $newsletterContentArray = NewsletterEdition::prepareImageInclude( $newsletterContentArray );
        }
    ...
    static function prepareImageInclude( $newsletterContentArray )
    {
        ...
        {
            // replace all image src from http => file:\\ezroot\ so NewsletterMailComposer will embed it into the mail message
            if( $id === 'html' )
            {
