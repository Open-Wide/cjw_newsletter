<?php
/**
 * File containing the NewsletterEditionSend class
 *
 ...
 * @package newsletter
 */
class NewsletterEditionSend extends eZPersistentObject
{
     ...
     * @return void
     */
    function NewsletterEditionSend( $row )
    {
    ...
    static function definition()
    {
        return array( 'fields' => array( 'id' => array( 'name' => 'Id',
                      ...
                      'class_name' => 'NewsletterEditionSend',
                      'name' => 'nl_edition_send' );
    }

    /**
     * Create new NewsletterEditionSend object
     *
     * @param NewsletterEdition $editionObject
     * @return object
     */
    static function create( NewsletterEdition $editionObject )
    {
        ...
        $rows = array(
                            ...
                            'status' => NewsletterEditionSend::STATUS_WAIT_FOR_PROCESS ,
                            'output_xml' => $outputXml,
                            'hash' => NewsletterUtils::generateUniqueMd5Hash( $listContentObjectId. $editionContentObjectId. $editionContentObjectVersion ),
        ...
        $object = new NewsletterEditionSend( $rows );
        return $object;
    }
    ...
    function setAttribute( $attr, $value )
    {
        ...
        switch ( $attr )
        {
            case 'status':
            {
                if ( $value === NewsletterEditionSend::STATUS_MAILQUEUE_CREATED )
                {
                    $this->setAttribute('mailqueue_created', time() );

                }
                elseif ( $value === NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_STARTED )
                {
                    $this->setAttribute('mailqueue_process_started', time() );
                }
                elseif ( $value === NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_FINISHED )
                {
                    $this->setAttribute('mailqueue_process_finished', time() );
                }
                elseif ( $value === NewsletterEditionSend::STATUS_ABORT )
                {
    ...
    static function fetchByEditionContentObjectIdVersion( $editionContentobjectId, $editionContentOjbectVersion, $asObject = true  )
    {
        $objectList = eZPersistentObject::fetchObjectList(
                        NewsletterEditionSend::definition(),
                        null,
    ...
    function getSendItemsStatistic()
    {
        ...
        $itemsCount = NewsletterEditionSendItem::fetchListBySendIdAndStatusCount( $editionSendId, false );
        $itemsNotSend = NewsletterEditionSendItem::fetchListBySendIdAndStatusCount( $editionSendId, NewsletterEditionSendItem::STATUS_NEW );
        $itemsSend = NewsletterEditionSendItem::fetchListBySendIdAndStatusCount( $editionSendId, NewsletterEditionSendItem::STATUS_SEND  );
        $itemsBounced = NewsletterEditionSendItem::fetchBounceCountByEditionSendId ( $editionSendId );
    ...
    function abortAllSendItems()
    {
        ...
        $db = eZDB::instance();
        $editionSendItemStatusAbort = NewsletterEditionSendItem::STATUS_ABORT;
        $query = "UPDATE `nl_edition_send_item` SET `status` = '$editionSendItemStatusAbort'
                  WHERE `nl_edition_send_item`.`edition_send_id` =$editionSendId
                  AND `nl_edition_send_item`.`status` =0;";

        $updateResult = $db->query( $query );

        // sendObject->set status on 9
        $this->setAttribute( 'status', NewsletterEditionSend::STATUS_ABORT );
        $this->store();
