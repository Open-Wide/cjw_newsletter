<?php
/**
 * File containing the NewsletterUser class
 *
...
class NewsletterUser extends eZPersistentObject
{
    /**
     *
    ...
    static function definition()
    {
        return array( 'fields' => array( 'id' => array( 'name' => 'Id',
                      ...
                      'class_name' => 'NewsletterUser',
                      'name' => 'nl_user' );
    }

    /**
     * Create new NewsletterUser object
     *
     * @param string $email
     ...
     * @return object
     */
    static function create( $email, $salutation, $firstName, $lastName, $organisation, $eZUserId, $status = NewsletterUser::STATUS_PENDING, $context = 'default' )
    {
        ...
        $rows = array( 'created' => time(),
                       ...
                       'hash' => NewsletterUtils::generateUniqueMd5Hash( $email ),
                       'remote_id' => 'nl:'. $context .':' . NewsletterUtils::generateUniqueMd5Hash( $email ),
                       'status' => $status );

        $object = new NewsletterUser( $rows );
        return $object;
    }
   ...
   * @param string $organisation,
   * @param int $eZUserId
   * @param int $newNewsletterUserStatus the status for new created Newsletter users NewsletterUser::STATUS_PENDING
   * @return object
   */
    static function createUpdateNewsletterUser( $email,
                                                ...
                                                $organisation,
                                                $eZUserId,
                                                $newNewsletterUserStatus = NewsletterUser::STATUS_PENDING,
                                                $context = 'default' )
    {
        ...
        if ( is_object( $existingNewsletterUserObject ) )
        {
            ...
            $userObject->store();
            NewsletterLog::writeDebug(
                                    'NewsletterUser::createUpdateNewsletterUser',
                                    'user',
                                    'update',
        ...
        {
            $userObject = NewsletterUser::create( $email,
                                                     $salutation,
                                                     $firstName,
                                                     $lastName,
                                                     $organisation,
    ...
    function setAttribute( $attr, $value )
    {
        ...
        switch ( $attr )
        {
            case 'email':
            {
                ...
                if ( $currentEmail != $newEmail )
                {
                    NewsletterLog::writeNotice(
                                        'set new email NewsletterUser::setAttribute',
                                        'user',
                                        'email',
            ...
            case 'status':
            {
                ...
                switch ( $value )
                {
                    case NewsletterUser::STATUS_CONFIRMED :
                    {
                    ...
                    case NewsletterUser::STATUS_BOUNCED_SOFT :
                    case NewsletterUser::STATUS_BOUNCED_HARD :
                    {
                ...
                {
                    NewsletterLog::writeNotice(
                                        'set NewsletterUser::setAttribute',
                                        'user',
                                        'status',
                                         array( 'nl_user' => $this->attribute( 'id' ),
    ...
    {
        $status = $this->attribute('status');
        if ( $status == NewsletterUser::STATUS_CONFIRMED )
            return true;
        else
            return false;
    }
    ...
    {
        $status = $this->attribute('status');
        if ( $status == NewsletterUser::STATUS_REMOVED_SELF )
            return true;
        else
            return false;
    }
    ...
    {
        $status = $this->attribute('status');
        $isOnBlacklist = NewsletterBlacklistItem::isEmailOnBlacklist( $this->attribute( 'email' ) );
        if( $isOnBlacklist )
        {
            // fix up status blacklisted if it is not set
            if( $status != NewsletterUser::STATUS_BLACKLISTED )
            {
                $this->setBlacklisted();
                return true;
            }
    ...
    function sendSubcriptionMail( $mailTemplate )
    {
        ...
        $emailSender = $NewsletterIni->variable( 'NewsletterMailSettings', 'EmailSender' );
        $emailSenderName = $NewsletterIni->variable( 'NewsletterMailSettings', 'EmailSenderName' );
        $emailReceiver = $newsletterUser->attribute('email');

        // TODO Namen extrahieren
        $emailReceiverName = '';
		
        $mailFormat = $NewsletterIni->hasVariable('NewsletterMailSettings','MailFormat')?$NewsletterIni->variable('NewsletterMailSettings','MailFormat'):'text';
        ...
        $Mail->setExtraMailHeadersByNewsletterUser( $this );
        $Mail->setTransportMethodDirectlyFromIni();

        // trigger_error("test error", E_USER_ERROR);
        $sendResult = $Mail->sendEmail( $emailSender,
                                          $emailSenderName,
                                          $emailReceiver,
                                          $emailReceiverName,
                                          $emailSubject,
    ...
    function getStatusString()
    {
        ...
        switch( $this->attribute('status') )
        {
            case self::STATUS_PENDING_EZ_USER_REGISTER:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Pending eZ User Register' );
                break;
            case self::STATUS_PENDING:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Pending' );
                break;
            case self::STATUS_CONFIRMED:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Confirmed' );
                break;
            case self::STATUS_REMOVED_SELF:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Removed by user' );
                break;
            case self::STATUS_REMOVED_ADMIN:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Removed by admin' );
                break;
            case self::STATUS_BOUNCED_SOFT:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Bounced soft' );
                break;
            case self::STATUS_BOUNCED_HARD:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Bounced hard' );
                break;
            case self::STATUS_BLACKLISTED:
                $statusString = ezpI18n::tr( 'newsletter/user/status', 'Blacklisted' );
                break;
        }
        return $statusString;
    }
    ...
    function getSubscriptionArray()
    {
        ...
        $subscriptionArray = NewsletterSubscription::fetchSubscriptionListByNewsletterUserId( $this->attribute('id') );
    ...
    function confirmAll()
    {
        ...
        {
            $this->setAttribute( 'status', NewsletterUser::STATUS_CONFIRMED );
        }
        // all subscriptions
    ...
    {
        return eZPersistentObject::fetchObject( NewsletterUser::definition(),
                                                null,
                                                array( 'hash' => $hash ),
                                                $asObject );
    }
    ...
    static function fetchByEmail( $email )
    {
        ...
        elseif ( $count > 1 )
        {
            ...
            NewsletterLog::writeError(
                                    'email existing more than 1 time NewsletterUser::fetchByEmail',
                                    'user',
                                    'email',
                                     array(
    ...
    public static function fetchListCount( $emailSearch = false )
    {
        ...
        $query = 'SELECT COUNT(id) AS count FROM nl_user' .$customConds;
     ...
     * @param integer $offset
     * @param boolean $asObject
     * @return array with NewsletterUser objects
     */
    ...
    {
        return NewsletterUser::getAvailableSalutationNameArrayFromIni();
    }
    /**
     *
     * @return array[salutation_id]=>i18n
    ...
    */
    ...
    function remove( $conditions = null, $extraConditions = null )
    {
        ...
        $blackListItem = NewsletterBlacklistItem::fetchByEmail( $this->attribute( 'email' ) );
     ...
     * Called from NewsletterBackListItem::store()
     * @return void
     */
    public function setBlacklisted()
    {
        NewsletterLog::writeDebug(
                                    'NewsletterUser::setBlacklisted',
                                    'user',
                                    'blacklist',
                                     array( 'nl_user' => $this->attribute( 'id' ) )
                                     );
        ...
        // set all subscriptions and all open senditems to blacklisted
        $this->setAllNewsletterUserRelatedItemsToStatus( NewsletterSubscription::STATUS_BLACKLISTED );
    ...
    {
        NewsletterLog::writeDebug(
                                    'NewsletterUser::setNonBlacklisted',
                                    'user',
                                    'blacklist',
                                     array( 'nl_user' => $this->attribute( 'id' ) )
                                     );
        ...
        // set all subscriptions and all open senditems to blacklisted
        foreach( NewsletterSubscription::fetchSubscriptionListByNewsletterUserId( $this->attribute( 'id' ) )
            as $subscription )
        {
    ...
    private function setAllNewsletterUserRelatedItemsToStatus( $status )
    {
        ...
        switch ( $status )
        {
            case NewsletterSubscription::STATUS_BOUNCED_SOFT:
            case NewsletterSubscription::STATUS_BOUNCED_HARD:
            case NewsletterSubscription::STATUS_BLACKLISTED:

                // update active senditems
                $abortCount = NewsletterEditionSendItem::setAllActiveItemsToStatusAbortByNewsletterUserId( $newsletterUserId );

                // update active subscriptions
                $activeSubscriptionList = NewsletterSubscription::fetchListNotRemovedOrBlacklistedByNewsletterUserId( $newsletterUserId, true );
                foreach ( $activeSubscriptionList as $subscription )
                {
                    if( $subscription->attribute( 'status') == $status )
                    {
                        NewsletterLog::writeDebug(
                                                    'skip - already set this status - NewsletterUser::setAllNewsletterUserRelatedItemsToStatus',
                                                    'subscription',
                                                    'status',
                                                     array( 'status' => $status,
                    ...
                    else
                    {
                        ...
                        $subscription->store();
                    /*    NewsletterLog::writeDebug(
                                                    'set NewsletterUser::setAllNewsletterUserRelatedItemsToStatus',
                                                    'subscription',
                                                    'status',
                                                     array( 'status' => $status,
                                                            'subscription_id' => $subscription->attribute('id'),
     ...
     * negative value fail
     * @see NewsletterSubscription::createSubscriptionByArray
     */
    public static function checkIfUserCanBeUpdated( $email, $ezUserId, $updateNewEmail = false )
    {
        ...
        $idNlUser = NewsletterUser::fetchByEzUserId( $ezUserId );
        $emailNlUser = NewsletterUser::fetchByEmail( $email );
