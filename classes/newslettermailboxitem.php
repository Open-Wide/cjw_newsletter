<?php
/**
 * File containing the NewsletterMailboxItem class
 *
 ...
 * @package newsletter
 */
class NewsletterMailboxItem extends eZPersistentObject
{
    /**
     ...
     * @return void
     */
    function NewsletterMailboxItem( $row )
    {
    ...
    static function definition()
    {
        return array( 'fields' => array( 'id' => array( 'name' => 'ID',
                      ...
                      'keys' => array( 'id' ),
                      'increment_key' => 'id',
                      'class_name' => 'NewsletterMailboxItem',
                      'name' => 'nl_mailbox_item',
    ...
    /**
     * fetch NewsletterMailboxItem object by id
    ...
    public static function fetch( $id, $asObject = true )
    {
         return eZPersistentObject::fetchObject(
                                                    NewsletterMailboxItem::definition(),
                                                    null,
                                                    array( 'id' => (int) $id ),
                                                    $asObject
                                                );
    ...
    public static function addMailboxItem( $mailboxId, $messageIdentifier, $messageId, $messageString )
    {
        ...
        if ( !is_object( $foundMessage ) )
        {
            ...
            $newMailboxItemObject = new NewsletterMailboxItem( $row );
    ...
    {
        $mailParserObject = new NewsletterMailParser( $this );
    ...
    private function saveParsedInfos( $parsedResult )
    {
        ...
        // if x-nl-senditem hash was set in bounce mail than fetch some ez data
        if ( isset( $parsedResult[ 'x-nl-senditem' ] ) )
        {
            $sendItemHash = $parsedResult[ 'x-nl-senditem' ];

            // try to fetch edition send item object
            $sendItemObject = NewsletterEditionSendItem::fetchByHash( $sendItemHash, true );
        ...
        // if only set 'x-nl-user'
        elseif ( isset( $parsedResult[ 'x-nl-user' ] ) )
        {
            $newsletterUser = NewsletterUser::fetchByHash( $sendItemHash, true );
        ...
        NewsletterLog::writeDebug(
                                    'parse_result NewsletterMailboxItem::saveParsedInfos',
                                    'mailbox_item',
                                     $this->attribute( 'id' ),
                                     $parsedResult
                                     );
