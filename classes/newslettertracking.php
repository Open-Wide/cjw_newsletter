...
class NewsletterTracking
{
	...
	/**
     * Return new $NewsletterTrackingClass object. Return false if NewsletterTracking is disabled or if TrackingClass doesn't exists
     * Used in process cronjob.
     *
     * @return NewsletterTracking or $NewsletterTrackingClass
     */
	public static function create( ) {
		if ( self::isEnabled() ) {
			$ini = eZINI::instance( 'newsletter.ini' );
			if ( $ini->hasVariable( 'NewsletterTracking', 'TrackingClass' ) ) {
		    	$NewsletterTrackingClass = $ini->variable( 'NewsletterTracking', 'TrackingClass' );
				if ( class_exists( $NewsletterTrackingClass ) ) {
					return new $NewsletterTrackingClass( );
				}
			}
		}
		return false;
	...
	/**
     * Assign $newsletterUserObject to current instance of NewsletterTracking
     * Used in process cronjob.
     *
     * @param NewsletterUser $newsletterUserObject
     * @return boolean
     */	
	public function setNewsletterUserObject ($newsletterUserObject) {
		if ( $newsletterUserObject instanceof NewsletterUser ) {
	...
	/**
     * Assign object with ID $editionContentObjectID to current instance of NewsletterTracking
     * Used in process cronjob.
     *
	...
	/**
     * Return current NewsletterList alias, from url_alias of current newsletter
	...
	protected function getUserID() {
		if ( $this->newsletterUserObject instanceof NewsletterUser ) {
	...
	/**
     * Return value of current NewsletterEdition attribute, identified by $identifier
     * Used in placeholders replacement.
     *
