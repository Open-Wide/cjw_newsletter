<?php
/**
 * File containing the NewsletterLog class
 *
 ...
 */
/**
 * Log handling of the  Newsletter using ezcLog
 * logmessage
 * time [severity] [source] [category] ( element1: value1, .... )
 *
 * <code>
 *     $log = NewsletterLog::getInstance();
 *     $log->source = "source";
 ...
 * @package newsletter
 */
class NewsletterLog extends ezcLog
{
    /**
    ...
    /**
     * Constructs an empty NewsletterLog instance
     *
    ...
    public static function getInstance( $isCliMode = false )
    {
        try {
            if ( is_null( self::$instance ) )
            {
                ...
                ezcBaseInit::fetchConfig( 'NewsletterInitLog', self::$instance );
            }
            return self::$instance;
        }
        catch ( ezcBaseFilePermissionException $e )
        {
            eZDebug::writeError( $e->getMessage(), 'NewsletterLog::getInstance()' );
            if ( $isCliMode )
            {
