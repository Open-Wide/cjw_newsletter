<?php
/**
 * File containing the NewsletterMailbox class
 *
 ...
 * @package newsletter
 */
class NewsletterMailbox extends eZPersistentObject
{
                    ...
                    try
                    {
                        ...
                        if( is_object( $connectResult ) )
                        {
                            NewsletterLog::writeDebug(
                                                    'NewsletterMailbox::collectMailsFromActiveMailboxes',
                                                    'mailbox',
                                                    'connect-ok',
    ...
    public function connect()
    {
        ...
        {
            NewsletterLog::writeError(
                                            'authenticate ezcMailTransport NewsletterMailbox::connect',
                                            'mailbox',
                                            'connect-failed',
                                             array( 'error-code' => $e->getMessage() ) );
            return $e->getMessage();
            ...
            if ( count( $messageNumberArray ) > 0 )
            {
                ...
                foreach ( $messageNumberArray as $messageId => $messageIdentifier )
                {
                    ...
                    else
                    {
                        ...
                        {
                            $messageIdentifier = 'nl_'.md5( $messageString );
                        }
                        ...
                        if ( $messageString != null )
                        {
                            ...
                            $addResult = NewsletterMailboxItem::addMailboxItem( $mailboxId, $messageIdentifier, $messageId, $messageString );
    ...
    private function extractAllExistingIdentifiers( $messageIdentifierArray )
    {
        ...
        $db = eZDB::instance();
        $sql = "SELECT id, message_identifier FROM nl_mailbox_item
