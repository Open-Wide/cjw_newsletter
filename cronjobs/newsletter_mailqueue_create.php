 ...
 *
 * Create all nl_edition_send_items (all subscribers of the list linked to the edtion)
 * for a newsletter edition which is waiting to send out
 *
 * - search all nl_users with status STATUS_PENDING_EZ_USER_REGISTER ( if subscribing in ez user register process )
 *   and if it is enabled => confirm the relationg nl_user + subscriptions
 *
 * -search all nl_edition_send object with status == STATUS_WAIT_FOR_PROCESS
 * -seach all nl_user which are in the list with status == STATUS_APPROVED
 * -create for every user a nl_edition_send_item with status == STATUS_NOT_SEND
...
$message = "START: newsletter_mailqueue_create";
$cli->output( $message );

// before we create the edition_send_items we will activate all nl_user with statua 20 - NewlsetterUser::STATUS_PENDING_EZ_USER_REGISTER

$message = "--\n>> START: check nl users with status STATUS_PENDING_EZ_USER_REGISTER";
$cli->output( $message );

$pendingNlUserObjectArray = NewsletterUser::fetchUserListByStatus( NewsletterUser::STATUS_PENDING_EZ_USER_REGISTER, 50, 0, true );
...
foreach ( $pendingNlUserObjectArray as $nlUser )
{
    ...
    else
    {
        ...
        $nlUser->setAttribute( 'status', NewsletterUser::STATUS_PENDING );
...
$waitForProcessObjectList = NewsletterEditionSend::fetchEditionSendListByStatus( array( NewsletterEditionSend::STATUS_WAIT_FOR_PROCESS ) );
...
foreach ( $waitForProcessObjectList as $newsletterEdtionSendObject )
{
    ...
    if ( $editionContentObjectTimestamp < time()) {
	    ...
	    $message = "## Procsessing: newsletter_mailqueue_create - sendObjectId: ". $sendId;
	    $cli->output( $message );
	
	    // 3. search all user which corresponding with list and has NewslettersSubscription::STATUS_APPROVED
	    // create a new send_item-entry
	    $limit = 0;
	    $offset = 0;
	    $subscriptionObjectList = NewsletterSubscription::fetchSubscriptionListByListIdAndStatus( $listContentObjectId, NewsletterSubscription::STATUS_APPROVED, $limit, $offset  );
	    ...
	    foreach ( $subscriptionObjectList as $subscriptionObject )
	    {
	        ...
	        if ( $newsletterEdtionSendObject->attribute('status') == NewsletterEditionSend::STATUS_WAIT_FOR_PROCESS )
	        {
	            ...
	            {
	                $newSendItemResult = NewsletterEditionSendItem::create( $sendId,
	                                                                     $newsletterUserId,
	                                                                     $outputFormatId,
	                                                                     $subscriptionId );
	    ...
	    $cli->output( $message );
	    $newsletterEdtionSendObject->setAttribute('status', NewsletterEditionSend::STATUS_MAILQUEUE_CREATED );
	    $newsletterEdtionSendObject->store();
	}
}
...
$message = "END: newsletter_mailqueue_create";
$cli->output( $message );
