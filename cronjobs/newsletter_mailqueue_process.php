...
/**
 * Cronjob newsletter_create_mailqueue.php
 *
 * Send out all newsletter mails - which are in the mailqueue
 *
 * - search all nl_edition_send_items, which are not send
 * - process every nl_edition_send_item separatly
...
$message = "START: newsletter_mailqueue_process";
$cli->output( $message );

// to fetch all send objetc with status == STATUS_MALQUEUE_CREATED || STATUS_MALQUEUE_STARTED
$sendObjectList = NewsletterEditionSend::fetchEditionSendListByStatus( array( NewsletterEditionSend::STATUS_MAILQUEUE_CREATED , NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_STARTED ) );

// Create newsletterTracking object
$NewsletterTracking = NewsletterTracking::create();
...
foreach ( $sendObjectList as $sendObject )
{
    ...
    if ( $sendObject->attribute( 'status' ) == NewsletterEditionSend::STATUS_MAILQUEUE_CREATED )
    {
        // if ok, set status == STATUS_MAILQUEUE_PROCESS_STARTED
        $sendObject->setAttribute('status', NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_STARTED );
        $sendObject->store();
	...
	if ( $NewsletterTracking ) {
    	$NewsletterTracking->setEditionContentObject( $sendObject->attribute('edition_contentobject_id') );
    }
    ...
    foreach( $outputFormatStringArray as $outputFormatId => $outputFormatNewsletterContentArray )
    {
       ...
       {
           $outputFormatStringArray[ $outputFormatId ] = NewsletterEdition::prepareImageInclude( $outputFormatNewsletterContentArray );
       }
    }
    // embed images ends
    ...
    $limit = $Ini->variable('NewsletterMailSettings', 'EmailsBySendingSession');
    ...
    $Mail->setTransportMethodCronjobFromIni();

    // process every send_item of current sendobject
    for( $i = 0; $i < $itemsNotSend; $i += $limit)
    {
      //  $progressBar->advance();
        $sendItemList = NewsletterEditionSendItem::fetchListSendIdAndStatus( $editionSendId, NewsletterEditionSendItem::STATUS_NEW, $limit, $offset );
        ...
        foreach ( $sendItemList as $sendItem )
        {
        	...
        	if ( $NewsletterTracking ) {
		    	$NewsletterTracking->setNewsletterUserObject( $newsletterUserObject );
		    }
            ...
            foreach ( $outputStringArray as $index => $string )
            {
				...
				if ( $NewsletterTracking ) {
					$string = $NewsletterTracking->insertMarkers( $string );
				}
            ...
            $Mail->setExtraMailHeadersByNewsletterSendItem( $sendItem );

            $resultArray = $Mail->sendEmail( $emailSender,
                                                $emailSenderName,
                                                $emailReceiver,
                                                $emailReceiverName,
                                                $emailSubject,
            ...
            if ( $sendResult === true )
            {
                ...
                $sendItem->setAttribute('status', NewsletterEditionSendItem::STATUS_SEND );
                $sendItem->store();
            }
            else
            {
        ...
        $cli->output( $message );
        usleep( $Ini->variable('NewsletterMailSettings', 'DelayBeforeNextSendingSession')*1000000 );
    }
    ...
    if ( $itemsCountAll == $itemsSend )
    {
        ...
        $sendObject->setAttribute('status', NewsletterEditionSend::STATUS_MAILQUEUE_PROCESS_FINISHED  );
        $sendObject->store();
...
$message = "END: newsletter_mailqueue_process";
$cli->output( $message );
