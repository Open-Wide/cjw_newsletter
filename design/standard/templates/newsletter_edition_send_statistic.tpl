...
<h2 class="context-title">{'Newsletter Edition send out statistic'|i18n( 'newsletter/newsletter_edition_send_statistic' )}</h2>
...
<p>{"Current Date"|i18n("newsletter/newsletter_edition_send_statistic")}: {currentdate()|l10n( 'datetime' )}</p>


        <table class="list" cellspacing="0">
        <tr>
            <th>{'Id'|i18n( 'newsletter/newsletter_edition_send_statistic' )}</th>
            <th>{'Mail count'|i18n( 'newsletter/newsletter_edition_send_statistic' )}</th>
            <th>{'Mail send'|i18n( 'newsletter/newsletter_edition_send_statistic' )}</th>
            <th>{'Mail not send'|i18n( 'newsletter/newsletter_edition_send_statistic' )}</th>
            <th>{'Mail bounced'|i18n( 'newsletter/newsletter_edition_send_statistic' )}</th>
        ...
        {foreach $newsletter_edition_attribute_content.edition_send_array.all as $edition_send_object}
         <tr>
            ...
            {if or( $edition_send_object.status|eq(3), $edition_send_object.status|eq(9) )}
                {'Abort cronjob'|i18n( 'newsletter/newsletter_edition_send_statistic' )}
            {else}
