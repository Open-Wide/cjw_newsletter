<div class="block">
    ...
    <table cellspacing="0" cellpadding="0" class="list">
        <tr>
            <th width="61%" class="name" colspan="2">{*<a href={$newsletter_system_node.url_alias|ezurl} title="{$newsletter_system_node.name|wash()}">{'newsletter_system'|class_icon( 'small' )}</a> *}{$newsletter_system_node.name|wash()}</th>
        </tr>
        ...
        {foreach $newsletter_list_node_list as $newsletter_list_node sequence array( 'bglight', 'bgdark' ) as $style}
            ...
            <tr class="{$style}">
                <td width="61%" align="left">{'newsletter_list'|class_icon( 'small' )} <a href={$newsletter_list_node.url_alias|ezurl}>{$newsletter_list_node.name|wash()} / {$newsletter_list_node.data_map.title.content|wash()}</a></td>
                <td align="left" width="39%" nowrap>
                    ...
                    <form action={'content/action'|ezurl()} name="CreateNewNewsletterEdition" method="post">
                        ...
                        <input type="hidden" value="newsletter_edition" name="ClassIdentifier"/>
