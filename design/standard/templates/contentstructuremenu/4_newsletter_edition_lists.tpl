...
alle mailing lists
*}
{let $newsletter_root_node_id = $newsletter_edition_lists_node.node_id
...
<li id="n0_{$newsletter_root_node_id}" {cond( $:last_item, 'class="lastli"', '' )}>
	...
	{if or( eq($ui_context, 'browse')|not(), eq($:parentNode.object.is_container, true()))}
		<a class="nodetext" href={concat( 'content/view/full/',$newsletter_root_node_id )|ezurl} title="{$:toolTip}"><span class="node-name-normal">{$newsletter_edition_lists_node.name}</span></a>
	{else}
		<span class="node-name-normal">{$newsletter_edition_lists_node.name}</span>
	{/if}
