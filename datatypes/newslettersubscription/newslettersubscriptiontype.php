<?php
/**
 * File containing the NewsletterSubscriptionType class
 *
...
class NewsletterSubscriptionType extends eZDataType
{
    const DATA_TYPE_STRING = 'newslettersubscription';

    function NewsletterSubscriptionType()
    {
        $this->eZDataType( self::DATA_TYPE_STRING, ezpI18n::tr( 'newsletter/datatypes', ' Newsletter Subscription', 'Datatype name' ),
        ...
        if ( isset( $dataArray[ 'subscription_data_array' ] ) )
        {
            if( isset( $dataArray[ 'subscription_data_array' ][ 'list_array' ] ) )
            {
                ...
                if( count( $listArray ) > 0 )
                {
                    ...
                    $subscriptionResultArray = NewsletterSubscription::createSubscriptionByArray( $dataArray[ 'subscription_data_array' ],
                                                                                                     NewsletterUser::STATUS_PENDING_EZ_USER_REGISTER,
                                                                                                     false,
                                                                                                     $context );
    ...
    function validateObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        else
        {
            ...
            {
                $contentObjectAttribute->setValidationError( ezpI18n::tr( 'newsletter/datatype/newslettersubcription/validation_error',
            ...
            $checkResult = NewsletterUser::checkIfUserCanBeUpdated( $email, $ezUserId, $updateNewEmail = false );
    ...
    private function fetchCurrentDataFromCollection( $collection )
    {
        ...
        if( count( $subscriptionDataArr['list_array'] ) > 0 )
        {
            ...
            else
            {
                ...
                $subscriptionResultArray = NewsletterSubscription::createSubscriptionByArray(
                                                                            $subscriptionDataArr,
    ...
    private function fetchCurrentContentObjectData( &$contentObjectAttribute )
    {
        ...
        {
            $existingNewsletterUser = NewsletterUser::fetchByEzUserId( $subscriptionDataArr['ez_user_id'] );
            if( is_object( $existingNewsletterUser ) === false )
            {
                if ( $subscriptionDataArr['email'] != '' )
                {
                    $existingNewsletterUser = NewsletterUser::fetchByEmail( $subscriptionDataArr['email'] );
                }
            }
        }
...
eZDataType::register( NewsletterSubscriptionType::DATA_TYPE_STRING, 'NewsletterSubscriptionType' );
?>
